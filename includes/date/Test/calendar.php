<?php
	// #########################################
	// Sprachsetup für Kalendercontrol:
	...
	function calendar($ux, $dx, $date = '', $link_before_date = 0, $link_after_date = 1)
	{
	    ...
	    $param_d['link_after_date']        = $link_after_date; // Enable link on days after the current day
	    $param_d['link_before_date']    = $link_before_date; // Enable link on days before the current day

	    $param_d['link_on_day']            = $PHP_SELF.'?date=%%dd%%&dx='.$dx.'&ux='.$ux.''; // Link to put on each day
	    ...
	    $param_d['font_nav_bg_color']    = '#CC3300';
	    ...
	    if ($param['show_month'] == 1)
	    {
	        ...
	        if ($param['use_img'] ) {
	            $output .= '<img src="../includes/date/mois.gif">';
	        }
	        if ( $param['day_mode'] == 1 ) {
	    ...
	    if ($param['show_day'] == 1 && $param['day_mode'] == 0) {
	        ...
	        for ($i = $first_day; $i < 7 + $first_day; $i++) {
	            ...
	            $day_name = ( $param['short_day_name'] == 1 ) ? substr($calendar_txt[$param['lang']]['days'][$index], 0, 2) : $calendar_txt[$param['lang']]['days'][$index];
	    ...
	    if ($param['nav_link'] == 1) {
	        ...
	        if ($param['use_img']) {
	            $g     = '<img src="../includes/date/g.gif" border="0">';
	            $gg = '<img src="../includes/date/gg.gif" border="0">';
	            $d     = '<img src="../includes/date/d.gif" border="0">';
	            $dd = '<img src="../includes/date/dd.gif" border="0">';
	        }
	        else {
	            $g     = '&lt;';
	            $gg = '&lt;&lt;';
