...
{
	echo "<SPAN CLASS=red_head>Aktuelle Downloads zur 100-Jahre-Feier:</SPAN><HR noshade size=1>".chr(13).chr(10);
	...
	while ($curfile = readdir ($handle))
	{
		...
		if ($curfile != ".")
		{
			if ($curfile != "..")
			{
				...
				$strSQL = $strSQL."AND modifieddate IS NULL AND category='100-Jahre-Feier'";
		...
		if ($filesfound=="FALSE" && $curfile != "." && $curfile != "..")
		{
			if ($messagedone=="FALSE")
			{
				...
				echo "<SPAN CLASS=sm>Es stehen derzeit keine Downloads zur Verf&uuml;gung.</SPAN><BR>".chr(13).chr(10);
				echo "</td></tr></table><BR>".chr(13).chr(10);
	...
	if (($filesfound=="FALSE") || ($onefilewriten=="FALSE"))
	{
		if ($messagedone=="FALSE")
		{
			echo "<table><tr><td>".chr(13).chr(10);
