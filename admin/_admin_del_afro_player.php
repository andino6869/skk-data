...
<?php
	writeheader("Admin - Gemeldeten Spieler im AFRO - Turnier l&ouml;schen");
?>
...
<?php
	...
	$strSQL = "select * from skk_afro_players WHERE del='N' AND modifieddate IS NULL AND curyear >= $curYear ORDER BY surname DESC, firstname, curyear ASC;";
	...
	if ($RecordCount > 0)
	{
		echo "<form method=post action='_admin_del_afro_player_ok.php'>".chr(13).chr(10);
		...
		echo "<TR><TD width='100%' bgcolor='#C0C0C0'><B><U>Gemeldeter Spieler:</U></B>";

		// Hilfe anzeigen?
		if ($dx==1)
		{
			echo "<BR><BR>".chr(13).chr(10);
			echo "<I>W&auml;hlen Sie hier den Spieler aus, den Sie l&ouml;schen m&ouml;chten.</I>";
		}
		...
		while ($row = $rs->fetch_object())
		{
	  		...
	  		echo "<OPTION Value='",$ID[$i],"'>",$surname[$i]." ".$firstname[$i].", geboren am ";
		...
		echo "<BR><INPUT TYPE=submit Value='Spieler entfernen'>".chr(13).chr(10);
		...
		if ($dx==1)
		{
			...
			echo "werden die Daten dieses Spielers f&uuml;r das betroffene Jahr aus der Tabelle 'skk_afro_players' gel&ouml;scht!<BR><BR>".chr(13).chr(10);
		}
	...
	else
	{
		...
		echo "In der Tabelle 'skk_afro_players' befinden sich derzeit keine bearbeitbaren Datens&auml;tze.<BR>".chr(13).chr(10);
	}
