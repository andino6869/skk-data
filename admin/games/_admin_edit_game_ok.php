...
<?php
	writeheader("Admin - Partie bearbeiten - Check", "TRUE");
?>
...
<?php
	...
	{
		// Partie aktualisieren:
		$now = date("Y-m-d H:i:s");

		$strSQL = "UPDATE skk_games SET modifieddate='".$now."', modifier = '".$curUser."' ";
		$strSQL = $strSQL."WHERE ID= ".$ID." AND modifieddate IS NULL AND del='N';";

		echo "<SPAN CLASS=he1>Partie aktualisieren (GAMES - Tabelle)</SPAN><BR><BR>".chr(13).chr(10);

		if (!mysqli_query ($objDBCon, $strSQL))
		{
			echo "Partie konnte nicht aktualisiert werden!<BR>".chr(13).chr(10);
			echo mysqli_error($objDBCon).chr(13).chr(10);
			echo "<BR>Statement: " . $strSQL . "<BR>".chr(13).chr(10);
		}
		else
		{
			$strSQL = "insert into skk_games (id, player1, player2, club1, club2, event, ";
			$strSQL = $strSQL."round, result, board, comment, password, moves, gamedate, creator, createdate, del) VALUES ";
			$strSQL = $strSQL."($ID, '$player1','$player2', $club1, $club2,'$event','$round', '$result', ";
			...
			{
		  		echo "Vielen Dank f&uuml;r das Aktualisieren der Partie!<BR>".chr(13).chr(10);
		  		echo "Die Partie ist jetzt wieder online.<BR><BR>".chr(13).chr(10);
